use piston_window::types::Color;
use piston_window::*;

use rand::{thread_rng, Rng};

use crate::draw::{draw_block, draw_rectangle};
use crate::snake::{Direction, Snake};

const FOOD_COLOR: Color = [0.60, 0.00, 0.00, 1.00];
const BORDER_COLOR: Color = [0.00, 0.00, 0.00, 1.00];
const GAMEOVER_COLOR: Color = [0.60, 0.00, 0.00, 0.50];
const WON_COLOR: Color = [0.00, 0.60, 0.00, 0.50];

const MOVING_TIME: f64 = 0.06;
const RESTART_TIME: f64 = 2.0;

pub struct Game {
    snake: Snake,

    food_exists: bool,
    food_x: i32,
    food_y: i32,

    width: i32,
    height: i32,

    game_over: bool,
    has_won: bool,
    waiting_time: f64,
}

impl Game {
    pub fn new(width: i32, height: i32) -> Game {
        Game {
            snake: Snake::new(1, 1),
            food_exists: true,
            food_x: 2,
            food_y: 2,
            width,
            height,
            game_over: false,
            has_won: false,
            waiting_time: 0.0,
        }
    }

    pub fn key_pressed(&mut self, key: Key) {
        if self.game_over | self.has_won {
            return;
        }

        let dir = match key {
            Key::Up | Key::K => Some(Direction::Up),
            Key::Down | Key::J => Some(Direction::Down),
            Key::Left | Key::H => Some(Direction::Left),
            Key::Right | Key::L => Some(Direction::Right),
            _ => None,
        };

        if dir.unwrap() == self.snake.head_direction().opposite() {
            return; // don't let a snake backtrack on itself
        }
        self.update_snake(dir);
    }

    pub fn draw(&self, ctx: &Context, g: &mut G2d) {
        self.snake.draw(ctx, g);

        if self.food_exists {
            draw_block(FOOD_COLOR, self.food_x, self.food_y, ctx, g);
        }

        draw_rectangle(BORDER_COLOR, 0, 0, self.width, 1, ctx, g);
        draw_rectangle(BORDER_COLOR, 0, self.height - 1, self.width, 1, ctx, g);
        draw_rectangle(BORDER_COLOR, 0, 0, 1, self.height, ctx, g);
        draw_rectangle(BORDER_COLOR, self.width - 1, 0, 1, self.height, ctx, g);

        if self.has_won {
            draw_rectangle(WON_COLOR, 0, 0, self.width, self.height, ctx, g);
        } else if self.game_over {
            draw_rectangle(GAMEOVER_COLOR, 0, 0, self.width, self.height, ctx, g)
        }
    }

    pub fn update(&mut self, delta_time: f64) {
        self.waiting_time += delta_time;

        if self.game_over || self.has_won {
            if self.waiting_time > RESTART_TIME {
                self.restart();
            }
            return;
        }

        if !self.food_exists {
            self.add_food();
        }

        if self.waiting_time > MOVING_TIME {
            self.update_snake(None);
        }
    }

    fn check_found_food(&mut self) {
        let (head_x, head_y): (i32, i32) = self.snake.head_position();
        if self.food_exists && self.food_x == head_x && self.food_y == head_y {
            self.food_exists = false;
            self.snake.restore_tail();
        }
    }

    fn check_has_won(&self) -> bool {
        self.snake.length() == (self.width - 2) * (self.height - 2)
    }

    fn check_if_snake_alive(&self, dir: Option<Direction>) -> bool {
        let (next_x, next_y) = self.snake.next_head(dir);

        if self.snake.has_collided(next_x, next_y) {
            return false;
        }

        next_x > 0 && next_y > 0 && next_x < self.width - 1 && next_y < self.height - 1
    }

    fn add_food(&mut self) {
        let mut rng = thread_rng();

        let mut new_x = rng.gen_range(1..self.width - 1); // don't place food on borders
        let mut new_y = rng.gen_range(1..self.height - 1);

        while self.snake.has_collided(new_x, new_y) {
            new_x = rng.gen_range(1..self.width - 1);
            new_y = rng.gen_range(1..self.height - 1);
        }

        self.food_x = new_x;
        self.food_y = new_y;
        self.food_exists = true;
    }

    fn update_snake(&mut self, dir: Option<Direction>) {
        if self.check_if_snake_alive(dir) {
            self.snake.move_forward(dir);
            self.check_found_food();

            if self.check_has_won() {
                self.has_won = true;
            }
        } else {
            self.game_over = true;
        }
        self.waiting_time = 0.0;
    }

    fn restart(&mut self) {
        self.snake = Snake::new(1, 1);
        self.waiting_time = 0.0;
        self.food_exists = true;
        self.food_x = 2;
        self.food_y = 2;
        self.game_over = false;
        self.has_won = false;
    }
}
